...
def main():
 ...
 # KRACK+ Attack options
    ...
    parser.add_option("--group", help="Only perform attack on the group key handshake", dest='group', action='store_true') #TODO implement this!
    ...
    if options.scan and not options.attack:
        ...
        try:
            ...
            with click.progressbar(range(50000), label="Cleaning up and generating PDF") as bar:
                for i in bar:
                    pass
            subprocess.call(["./restoreClientWifi.sh > /dev/null"])
            writeResults()
    ...
    ############# ATTACK ################
        ...
        # KeyBoardInterrupt exceptions occurs when the user presses Ctrl+C or user attempts to use invalid options
            ...
            subprocess.call(["./killProcesses.sh dnsmasq > /dev/null"], shell=True)
            ...
            if options.sslstrip:
                subprocess.call(["./killProcesses.sh sslstrip > /dev/null"], shell=True)
